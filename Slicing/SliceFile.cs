        ...
        public Bitmap GetSliceImage(int layer) // 0 based index
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        if (UVDLPApp.Instance().m_buildparms.m_exportopt.ToUpper().Contains("ZIP"))
                        {
                            // try first to load from zip
                            // read the bitmap from the zip
                            m_zip = ZipFile.Read(path);
                            string fname = String.Format("SEC_{0:0000}.gif", layer+1);
                        ...
                        {
                            path = GetSliceFilePath(UVDLPApp.Instance().SceneFileName);
                            string fname = String.Format("SEC_{0:0000}.gif", layer+1);
                            Bitmap bmp = new Bitmap(Path.Combine(path, fname));
                            return bmp;
                        }
                    }
                    catch (Exception ex )
