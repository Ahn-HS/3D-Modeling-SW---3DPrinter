...
namespace UV_DLP_3D_Printer.GUI.CustomGUI
{
    public partial class ctlObjectInfo : ctlAnchorable
    {
        ...
        public void FillObjectInfo(Object3d obj)
        {
            ...
            //by sypark 2016-02-26
            if (obj.m_min.x <= -1000)
            {
                tMin.DataText = String.Format("#####, {1:0.00}, {2:0.00}", obj.m_min.x, obj.m_min.y, obj.m_min.z);
                if (obj.m_min.y <= -1000)
                {
                    tMin.DataText = String.Format("#####, #####, {2:0.00}", obj.m_min.x, obj.m_min.y, obj.m_min.z);
                    if (obj.m_min.z <= -1000)
                        tMin.DataText = String.Format("#####, #####, #####", obj.m_min.x, obj.m_min.y, obj.m_min.z);
                }
                else if (obj.m_min.z <= -1000)
                {
                    tMin.DataText = String.Format("#####, {1:0.00}, #####", obj.m_min.x, obj.m_min.y, obj.m_min.z);
                }
            }
            else if (obj.m_min.y <= -1000)
            {
                tMin.DataText = String.Format("{0:0.00}, #####, {2:0.00}", obj.m_min.x, obj.m_min.y, obj.m_min.z);
                if (obj.m_min.z <= -1000)
                {
                    tMin.DataText = String.Format("{0:0.00}, #####, #####", obj.m_min.x, obj.m_min.y, obj.m_min.z);
                }
            }
            else if (obj.m_min.z <= -1000)
            {
                tMin.DataText = String.Format("{0:0.00}, {1:0.00}, #####", obj.m_min.x, obj.m_min.y, obj.m_min.z);
            }
            ...
            if (obj.m_max.x <= -1000)
            {
                tMax.DataText = String.Format("#####, {1:0.00}, {2:0.00}", obj.m_max.x, obj.m_max.y, obj.m_max.z);
                if (obj.m_max.y <= -1000)
                {
                    tMax.DataText = String.Format("#####, #####, {2:0.00}", obj.m_max.x, obj.m_max.y, obj.m_max.z);
                    if (obj.m_max.z <= -1000)
                        tMax.DataText = String.Format("#####, #####, #####", obj.m_max.x, obj.m_max.y, obj.m_max.z);
                }
                else if (obj.m_max.z <= -1000)
                {
                    tMax.DataText = String.Format("#####, {1:0.00}, #####", obj.m_max.x, obj.m_max.y, obj.m_max.z);
                }
            }
            else if (obj.m_max.y <= -1000)
            {
                tMax.DataText = String.Format("{0:0.00}, #####, {2:0.00}", obj.m_max.x, obj.m_max.y, obj.m_max.z);
                if (obj.m_max.z <= -1000)
                {
                    tMax.DataText = String.Format("{0:0.00}, #####, #####", obj.m_max.x, obj.m_max.y, obj.m_max.z);
                }
            }
            else if (obj.m_max.z <= -1000)
            {
                tMax.DataText = String.Format("{0:0.00}, {1:0.00}, #####", obj.m_max.x, obj.m_max.y, obj.m_max.z);
            }
            ...
            if (xs <= -1000)
            {
                tSize.DataText = String.Format("#####, {1:0.00}, {2:0.00}", xs, ys, zs);
                if (ys <= -1000)
                {
                    tSize.DataText = String.Format("#####, #####, {2:0.00}", xs, ys, zs);
                    if (zs <= -1000)
                        tSize.DataText = String.Format("#####, #####, #####", xs, ys, zs);
                }
                else if (zs <= -1000)
                {
                    tSize.DataText = String.Format("#####, {1:0.00}, #####", xs, ys, zs);
                }
            }
            else if (ys <= -1000)
            {
                tSize.DataText = String.Format("{0:0.00}, #####, {2:0.00}", xs, ys, zs);
                if (zs <= -1000)
                {
                    tSize.DataText = String.Format("{0:0.00}, #####, #####", xs, ys, zs);
                }
            }
            else if (zs <= -1000)
            {
                tSize.DataText = String.Format("{0:0.00}, {1:0.00}, #####", xs, ys, zs);
            }
